...
class GuiManager:
    ...
    def lf_criar_tela(self, args):
        """Cria uma tela de desenho (Canvas)."""
        id, largura, altura, cor_fundo = args
        tela = tk.Canvas(self.root, width=int(largura), height=int(altura), bg=str(cor_fundo))
        tela.pack(pady=10, padx=10)
        self.widgets[id] = tela

    def lf_vincular_evento_rato(self, args):
        """Vincula eventos do rato a funções Lucida-Flow."""
        id_widget, nome_evento, nome_funcao_lf = args
        if id_widget in self.widgets:
            widget = self.widgets[id_widget]
            
            # O 'lambda' captura o evento e passa as coordenadas x e y
            # para a nossa função que executa o código Lucida-Flow
            callback = lambda evento: run_lucida_function_com_args(
                nome_funcao_lf, [evento.x, evento.y]
            )
            
            # Mapeia nomes de eventos simples para os nomes técnicos do Tkinter
            eventos_tkinter = {
                "botao_pressionado": "<Button-1>",
                "botao_solto": "<ButtonRelease-1>",
                "movimento": "<B1-Motion>", # Movimento ENQUANTO o botão 1 está pressionado
            }
            
            if nome_evento in eventos_tkinter:
                widget.bind(eventos_tkinter[nome_evento], callback)

    def lf_desenhar_linha(self, args):
        """Desenha uma linha numa tela."""
        id_tela, x1, y1, x2, y2, cor = args
        if id_tela in self.widgets and isinstance(self.widgets[id_tela], tk.Canvas):
            self.widgets[id_tela].create_line(int(x1), int(y1), int(x2), int(y2), fill=str(cor), width=3)
...
def run_lucida_function_com_args(function_name, args):
    """Executa uma função Lucida-Flow específica, passando argumentos Python."""
    process_obj = INTERPRETER.global_scope.get(function_name)
    if process_obj and isinstance(process_obj, Process):
        try:
            INTERPRETER.call_process(process_obj, args)
        except Exception as e:
            messagebox.showerror("Erro de Runtime na Lucida-Flow", str(e))
...
def setup_analyzer():
    ...
    gui_semantic_scope.define(BuiltInFunctionSymbol('criar_tela', params=[VarSymbol('id', string_type), VarSymbol('w', int_type), VarSymbol('h', int_type), VarSymbol('cor', string_type)], return_type=null_type))
    gui_semantic_scope.define(BuiltInFunctionSymbol('vincular_evento_rato', params=[VarSymbol('id', string_type), VarSymbol('evento', string_type), VarSymbol('callback', string_type)], return_type=null_type))
    gui_semantic_scope.define(BuiltInFunctionSymbol('desenhar_linha', params=[VarSymbol('id', string_type), VarSymbol('x1', int_type), VarSymbol('y1', int_type), VarSymbol('x2', int_type), VarSymbol('y2', int_type), VarSymbol('cor', string_type)], return_type=null_type))
...
def main():
    ...
    INTERPRETER.global_scope['gui'] = {
        ...
        "criar_tela": BuiltInFunction(name='criar_tela', python_callable=gui_manager.lf_criar_tela),
        "vincular_evento_rato": BuiltInFunction(name='vincular_evento_rato', python_callable=gui_manager.lf_vincular_evento_rato),
        "desenhar_linha": BuiltInFunction(name='desenhar_linha', python_callable=gui_manager.lf_desenhar_linha),
    }

    try:
        with open("paint.lf", 'r', encoding='utf-8') as f: source_code = f.read()
