...
class GuiManager:
    ...
    def lf_salvar_dialogo_ficheiro(self, args):
        """Abre uma janela de diálogo para o utilizador definir onde salvar um ficheiro."""
        # 'asksaveasfilename' é a função do Tkinter para isso
        caminho_ficheiro = filedialog.asksaveasfilename(
            defaultextension=".txt",
            filetypes=[("Text files", "*.txt"), ("All files", "*.*")]
        )
        return caminho_ficheiro if caminho_ficheiro else ""

    def lf_abrir_dialogo_ficheiro(self, args):
        """Abre uma janela de diálogo para o utilizador selecionar um ficheiro."""
        # 'askopenfilename' é a função do Tkinter para isso
...
def setup_analyzer():
    ...
    gui_semantic_scope.define(BuiltInFunctionSymbol('salvar_dialogo_ficheiro', params=[], return_type=string_type))
    gui_semantic_scope.define(BuiltInFunctionSymbol('abrir_dialogo_ficheiro', params=[], return_type=string_type))
...
def main():
    ...
    INTERPRETER.global_scope['gui'] = {
        ...
        "salvar_dialogo_ficheiro": BuiltInFunction(name='salvar_dialogo_ficheiro', python_callable=gui_manager.lf_salvar_dialogo_ficheiro),
         "abrir_dialogo_ficheiro": BuiltInFunction(name='abrir_dialogo_ficheiro', python_callable=gui_manager.lf_abrir_dialogo_ficheiro)
    }

    try:
        with open("bloco_de_notas.lf", 'r', encoding='utf-8') as f: source_code = f.read()
